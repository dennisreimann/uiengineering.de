...
module.exports = (streamFilePath, siteHost, assetHost, defaultScheme) => {
  ...
  return {
    ...
    shortIdToGerman (shortId) {
      if (shortId.startsWith('QND')) {
        const match = shortId.match(/QND(\d+)?/)
        const episode = match ? parseInt(match[1]) : null

        if (episode) return `Quick 'n dirty ${episode}`
        return 'Quick \'n dirty'
      } else {
