...
export default {
  createHelper(file, urlBase) {
    const filePath = path.relative(paths.src, file.path);

    return {
      baseUrl(filePath, forceHttp) {
        let base = urlBase;
        if (forceHttp && base.startsWith('https')) {
          base = base.replace('https://', 'http://')
        }
        return !filePath.match(/^https?:/) ? base + filePath : filePath;
      },
      ...
      podcastImage(dir, size) {
        const folder = dir || 'cover';
        const file = revvedFile(`images/podcast/${folder}/${size}.png`);
        return `/${file}`;
      },
