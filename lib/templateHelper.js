...
export default {
  createHelper(streamFile, siteHost, assetHost) {
    const assetPath = (filePath) => `/${revvedFile(filePath)}`;

    const assetUrl = (filePath) => `//${assetHost || siteHost}${assetPath(filePath)}`;

    const siteUrl = (filePath, scheme) => `${scheme ? scheme + ':' : ''}//${siteHost}${filePath.startsWith('/') ? '' : '/'}${filePath}`;
    ...
    return {
      ...
      isCurrentPage(page) {
        const filePath = path.relative(paths.src, streamFile.path);
